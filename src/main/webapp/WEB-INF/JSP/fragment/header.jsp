...
<nav class="navbar navbar-default">
	<div class="container-fluid">
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="/products">IShop магазин</a>
		</div>
			...
			<ul id="currentShoppingCart"
				...
				<li class="dropdown"><a href="#" class="dropdown-toggle"
					...
					aria-expanded="false"> <i class="fa fa-shopping-cart"
						aria-hidden="true"></i> Shopping cart (<span class="total-count">${CURRENT_SHOPPING_CART.totalCount}</span>)<span
						class="caret"></span>
				</a>
			...
			<c:choose>
				<c:when test="${CURRENT_ACCOUNT != null }">
					<ul class="nav navbar-nav navbar-right">
						<li><img src="${CURRENT_ACCOUNT.avatarUrl == null ? '/media/avatar/default.jpg' : CURRENT_ACCOUNT.avatarUrl }" class="avatar"></li>
				...
				<c:when test="${CURRENT_REQUEST_URL != '/sign-in' and CURRENT_REQUEST_URL != '/shopping-cart' }">
					...
					<ishop:registration classes="navbar-btn navbar-right registration-in"/>
				</c:when>
			</c:choose>
		</div>
	</div>
